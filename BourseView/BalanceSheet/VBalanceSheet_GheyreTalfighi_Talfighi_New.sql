...
,cte2.[سود خالص تعدیل شده یک ساله]/(select sum([سود خالص تعدیل شده یک ساله]) from cte2) [درصد سود هر شرکت از کل سود شرکت ها]

,cast(cte2.['داراییهای ثابت مشهود'] / (select sum(['داراییهای ثابت مشهود']) from cte2)as decimal(30,7)) 
[سهم هر شرکت از کل دارایی ثابت شرکت ها]

,cte2.[فروش تعدیل شده یک ساله]/(select sum([فروش تعدیل شده یک ساله]) from cte2) [نسبت فروش هر شرکت از کل فروش شرکت ها]

,cast(cte2.['موجودی نقد'] / (select sum(['موجودی نقد']) from cte2)as decimal(30,7)) [سهم هر شرکت از کل وجوه نقد و موجودی نزد بانک ها],

cte2.[سود خالص تعدیل شده یک ساله]/(select sum([سود خالص تعدیل شده یک ساله]) from cte2) [درصد سود خالص هر شرکت از کل سود خالص شرکت ها],

cte2.[فروش تعدیل شده یک ساله]/(select sum([فروش تعدیل شده یک ساله]) from cte2) [درصد فروش هر شرکت از کل فروش شرکت ها]

,cte2.[خالص داریی جاری]/(select sum([خالص داریی جاری]) from cte2) [درصد خالص دارایی هر شرکت از کل دارایی شرکت ها]
...
,cast((cte2.['سرمایه گذاری در املاک'] + cte2.['سرمایه گذاری کوتاه مدت'] + cte2.['سرمایه گذاریهای بلند مدت'])/(select sum((['سرمایه گذاری در املاک'] + ['سرمایه گذاری کوتاه مدت'] + ['سرمایه گذاریهای بلند مدت'])) from cte2) as decimal(30,7))
...
case when cte2.GroupName = N'صنایع انرژی محور' then cte2.[سود خالص تعدیل شده یک ساله]/(select sum([سود خالص تعدیل شده یک ساله]) from cte2) else 0 end [درصد شرکت انرژی محور چند درصد از کل سود خالص]

,case when cte2.GroupName = N'معادن' then cte2.[سود خالص تعدیل شده یک ساله]/(select sum([سود خالص تعدیل شده یک ساله]) from cte2) else 0 end [درصد معادن چند درصد از کل سود خالص]
...
,case when cte2.GroupName = N'بانک' then( cte2.['سایر سپرده ها'] + cte2.['سپرده های سرمایه گذاری مدت دار'] + cte2.['سپرده های قرض الحسنه و پس اندار و مشابه'] + cte2.['سپرده های دیداری'])/(select sum(['سایر سپرده ها'] + ['سپرده های سرمایه گذاری مدت دار'] + ['سپرده های قرض الحسنه و پس اندار و مشابه'] + ['سپرده های دیداری']) from cte2) else 0 end
...
,case when cte2.GroupName = N'بانک' then cte2.['بدهی به بانکها و موسسات اعتباری']/(select sum(['بدهی به بانکها و موسسات اعتباری']) from cte2 where cte2.groupname = N'بانک') else 0 end 
...
,((cte2.['تسهیلات مالی دریافتی بلند مدت'] + cte2.['حصه جاری تسهیلات مالی دریافتی'])/(select sum(['تسهیلات مالی دریافتی بلند مدت'] + ['حصه جاری تسهیلات مالی دریافتی'] )from cte2))[درصد بدهی هر شرکت به بانک از کل شرکت ها]


,((cte2.['سود سهام پیشنهادی و پرداختنی'])/(select sum(['سود سهام پیشنهادی و پرداختنی']) from cte2))[درصد هر شرکت از کل سود پیشنهادی و پرداختی]
...
,cte2.['سود (زیان) عملیات در حال تداوم قبل از مالیات']
,cte2.['سود (زیان) پایه هر سهم ناشی از عملیات در حال تداوم - عملیاتی']
,cte2.['سود (زیان) پایه هر سهم ناشی از عملیات در حال تداوم']
,cte2.['بدهی بابت بیمه عمر و مدیریت سرمایه']
,cte2.['مطالبات از بیمه گران و بیمه‌گران اتکایی']
,cte2.['مطالبات از بیمه‌گذاران و نمایندگان']
,cte2.['سهم بیمه گران اتکایی از ذخایر فنی']
,cte2.['بدهی به بیمه‌گذاران و نمایندگان']
,cte2.['بدهی به بیمه‌گران و بیمه‌گران اتکایی']
,cte2.['ذخیره خسارت معوق ']
,cte2.['سایر ذخایر فنی']
,cte2.['ذخیره ریسک‌های منقضی نشده']
,cte2.['سود (زیان) خالص']
,cte2.['داراییهای نامشهود']
,cte2.['هزینه‌های مالی']
,cte2.['خالص سایر درآمدها و هزینه‌های غیرعملیاتی']
,cte2.['سایر درآمدهای عملیاتی']
,cte2.['سایر هزینه‌های عملیاتی']
,cte2.['سهم گروه از سود شرکت‌های وابسته و مشارکت‌های خاص'] 

,cte2.['هزینه مالیات بر درآمد سال جاری']
,cte2.['هزینه استهلاک']
,cte2.['هزینه های عمومی, اداری و تشکیلاتی']
,cte2.['هزینه های تولید']
,cte2.['هزینه سهم مشارکت در منافع']
,cte2.['سود (زیان) ناخالص فعالیتهای بیمه ای']
,cte2.['هزینه کاهش ارزش دریافتنی‌‏ها (هزینه استثنایی)']
,cte2.['هزینه تامین منابع مالی عملیات لیزینگ']
,cte2.['سایر هزینه های بیمه ای']
,cte2.['هزینه مالیات بر درآمد سال‌های قبل']
,cte2.['سایر درآمدها و هزینه‌های غیرعملیاتی - اقلام متفرقه']
,cte2.['هزینه ارائه خدمات']
,cte2.['درآمدهای عملیاتی']
,cte2.['بهای تمام ‌شده درآمدهای عملیاتی']
,cte2.['سود (زیان) عملیاتی']
from cte2
