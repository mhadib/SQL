  ...
  create     view  [dbo].[VBoursSpain_Denormal]  as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],null [شاخص بورس ۲۰۰۵ ],null [درصد تغییر نسبت به سال 2005],null as [رتبه بندی شرکت], null [سال ورود ], null [فعالیت شرکت ], null [معرفی شرکت] , nba.[رشد نسبت به سال 2005] [رشد نرخ برابری ارز نسبت به سال 2005], v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار],  v.InvestmentReturnPeriod [دوران بازگشت سرمایه]
  
  from BoursSpain b  
  --join BoursSpainHistorical bh on b.Name = bh.Name  
  left join NerkheBarabariHistory na on na.Name like N'%اسپانیا%'
  left join vNerkheBarabari nba on nba.PName like N'%'+dbo.RetreiveVahed(substring(na.pname,1,3))+'%'
  left join VBoursViewSpain v  on b.Name = v.symbol
  where not exists(select 1 from [dbo].BoursSpain b2 where b.Name = b2.Name and b2.createdate > b.createdate) )  
