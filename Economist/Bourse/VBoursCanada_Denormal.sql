...
CREATE   view  [dbo].[VBoursCanada_Denormal] as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],bh.[شاخص بورس کانادا  ۲۰۰۵ ] [شاخص بورس ۲۰۰۵ ],cast(cast((b.Gheymat - bh.[شاخص بورس کانادا  ۲۰۰۵ ]) /bh.[شاخص بورس کانادا  ۲۰۰۵ ] as decimal(10,4)) * 100 as decimal(10,2)) [درصد تغییر نسبت به سال 2005],null as [رتبه بندی   شرکت],bh.[نوع فعالیت] [فعالیت شرکت ], bh.[شرح فعالیت] [معرفی شرکت], bh.[سال ورود ] [سال ورود ],nba.[رشد نسبت به سال 2005] [رشد نرخ برابری ارز نسبت به سال 2005]  , v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار],  v.InvestmentReturnPeriod [دوران بازگشت سرمایه]
from BoursCanada b  
join BoursCanadaHistorical bh on b.Name = bh.Name  
left join NerkheBarabariHistory na on na.Name like N'%کانادا%'
left join vNerkheBarabari nba on nba.PName like N'%'+dbo.RetreiveVahed(substring(na.pname,1,3))+'%'
left join vBoursViewCanada v  on b.Name = v.symbol
where not exists(select 1 from [dbo].BoursCanada b2 where b.Name = b2.Name and b2.createdate > b.createdate) )  
