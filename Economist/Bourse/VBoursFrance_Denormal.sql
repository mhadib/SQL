    ...
    create   view  [dbo].[VBoursFrance_Denormal]    as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],bh.[شاخص بورس فرانسه  ۲۰۰۵ ] [شاخص بورس ۲۰۰۵ ],cast(cast((b.Gheymat - bh.[شاخص بورس فرانسه  ۲۰۰۵ ]) /bh.[شاخص بورس فرانسه  ۲۰۰۵ ]  as decimal(8,4)) * 100 as decimal(8,2)) [درصد تغییر نسبت به سال 2005],null as [رتبه بندی شرکت], bh.[سال ورود ] [سال ورود ], bh.[نوع فعالیت] [فعالیت شرکت ], bh.[شرح فعالیت] [معرفی شرکت]  ,nba.[رشد نسبت به سال 2005] [رشد نرخ برابری ارز نسبت به سال 2005], v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار],  v.InvestmentReturnPeriod [دوران بازگشت سرمایه]
	from BoursFrance b  
	join BoursFranceHistorical bh on b.Name = bh.Name  
	left join NerkheBarabariHistory na on na.Name like N'%فرانسه%'
	left join vNerkheBarabari nba on nba.PName like N'%'+dbo.RetreiveVahed(substring(na.pname,1,3))+'%'
	left join VBoursViewFrance v  on b.Name = v.symbol

	where not exists(select 1 from [dbo].BoursFrance b2 where b.Name = b2.Name and b2.createdate > b.createdate) )  
