...
--select ['سود (زیان) عملیات در حال تداوم قبل از مالیات'],['سود (زیان) خالص عملیات در حال تداوم قبل از مالیات'],['سود (زیان) خالص'],[سود خالص تعدیل شده یک ساله1],[سود خالص تعدیل شده یک ساله] ,* from VPreBalanceSheetGheyreTalfighi_Bourse
CREATE     view [dbo].[VPreBalanceSheetGheyreTalfighi_Bourse]
		as 
		with cte as 
	(
	select *,
...
cast(((cf.['جمع داراییهای جاری'] - cf.['جمع بدهیهای جاری'])) as decimal(10,0)) [خالص داریی جاری]
	from VPreBalanceSheetGheyreTalfighi_Bourse_Drop_Bilion cf
	), 
...
, cte.['هزینه‌های مالی']
, cte.['خالص سایر درآمدها و هزینه‌های غیرعملیاتی']
, cte.['سایر درآمدهای عملیاتی']
, cte.['سایر هزینه‌های عملیاتی']
, null ['سهم گروه از سود شرکت‌های وابسته و مشارکت‌های خاص'] 
, cte.['هزینه مالیات بر درآمد سال جاری']
, cte.['هزینه استهلاک']
, cte.['هزینه های عمومی, اداری و تشکیلاتی']
, cte.['هزینه های تولید']
, cte.['هزینه سهم مشارکت در منافع']
, cte.['سود (زیان) ناخالص فعالیتهای بیمه ای']
, cte.['هزینه کاهش ارزش دریافتنی‌‏ها (هزینه استثنایی)']
, cte.['هزینه تامین منابع مالی عملیات لیزینگ']
, cte.['سایر هزینه های بیمه ای']
, cte.['هزینه مالیات بر درآمد سال‌های قبل']
, cte.['سایر درآمدها و هزینه‌های غیرعملیاتی - اقلام متفرقه']
, cte.['هزینه ارائه خدمات']
, cte.['درآمدهای عملیاتی']
, cte.['بهای تمام ‌شده درآمدهای عملیاتی']
, cte.['سود (زیان) عملیاتی']
  FROM CTE 
--join cteSums on 1 = 1
