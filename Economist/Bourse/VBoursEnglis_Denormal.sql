...
--select * from VBoursEnglis_Denormal
  CREATE   view  [dbo].[VBoursEnglis_Denormal] as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],bh.[شاخص بورس انگلستان  ۲۰۰۵ ] [شاخص بورس ۲۰۰۵ ],cast(cast((b.Gheymat - bh.[شاخص بورس انگلستان  ۲۰۰۵ ]) /bh.[شاخص بورس انگلستان  ۲۰۰۵ ] as decimal(8,4)) * 100 as decimal(8,2))[درصد تغییر نسبت به سال 2005],null as [رتبه   بندی شرکت], bh.[سال ورود ] [سال ورود ], bh.[نوع فعالیت] [فعالیت شرکت ], bh.[شرح فعالیت] [معرفی شرکت] ,nba.[رشد نسبت به سال 2005] [رشد نرخ برابری ارز نسبت به سال 2005], v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار],  v.InvestmentReturnPeriod [دوران بازگشت سرمایه]
  
  from BoursEnglis b  
  join BourseEnglis2005 bh on b.Name = bh.Name  
  left join NerkheBarabariHistory na on na.Name like N'%انگلستان%'
  left join vNerkheBarabari nba on nba.PName like N'%'+dbo.RetreiveVahed(substring(na.pname,1,3))+'%'
  left join VBoursViewEngland v  on b.Name = v.symbol
  
  where not exists(select 1 from [dbo].BoursEnglis b2 where b.Name = b2.Name and b2.createdate > b.createdate) ) 
