...
,cast(hc.['ذخایر و سایر بدهی ها']/10000000000 as decimal(20,0)) ['ذخایر و سایر بدهی ها']
	...
	where not exists(select 1 from BalanceSheetTalfighiCategurbalancesheet_bourse h where hc.instrumentId = h.instrumentId and h.day > hc.day)
	and not exists(select 1 from BalanceSheetTalfighiCategurbalancesheet_bourse h where hc.instrumentId = h.instrumentId and h.createdate > hc.createdate)
