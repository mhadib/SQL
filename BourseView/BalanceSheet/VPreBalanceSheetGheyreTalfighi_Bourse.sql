...
--select * from VPreBalanceSheetGheyreTalfighi_Bourse
CREATE     view [dbo].[VPreBalanceSheetGheyreTalfighi_Bourse]
		as 
		with cte as 
	(
	select *,
	cast(((cf.['سود (زیان) عملیات در حال تداوم قبل از مالیات']/(cf.[ماه مالی]))*12) as decimal(30,4)) [سود خالص تعدیل شده یک ساله],
...
,cte.[خالص داریی جاری]
, cte.['جمع بدهیهای جاری و غیر جاری'] - (cte.['ذخیره مزایای پایان خدمت'] + cte.['ذخایر و سایر بدهی ها'] + cte.['بدهی به بانکها و موسسات اعتباری'] + cte.['سود سهام پیشنهادی و پرداختنی'])
['سپرده های مشتریان']
  FROM CTE 
--join cteSums on 1 = 1
