  ...
  CREATE view  [dbo].[VBoursUSNasdaq_Denormal]  as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],bh.[ارزش بورس نزدک  ۲۰۰۵ ] [شاخص بورس ۲۰۰۵ ],cast(cast((b.Gheymat - bh.[ارزش بورس نزدک  ۲۰۰۵ ]) /bh.[ارزش بورس نزدک  ۲۰۰۵ ] as decimal(8,4)) * 100 as decimal(8,2))[درصد تغییر نسبت به سال 2005],null as [رتبه بندی شرکت],   bh.[سال ورود ] [سال ورود ], bh.[نوع فعالیت] [فعالیت شرکت ], bh.[شرح فعالیت] [معرفی شرکت], v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار], v.InvestmentReturnPeriod [دوران بازگشت سرمایه]
  from BoursUSNasdaq b  
  join BoursUSNasdaqHistorical bh on b.Name = bh.Name  
  left join vBoursViewUSA v  on b.Name = v.symbol
  where not exists(select 1 from [dbo].BoursUSNasdaq b2 where b.Name = b2.Name and b2.createdate > b.createdate) )  
