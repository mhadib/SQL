...
--select  * from [dbo].[P_CalcDayCountInFloatAndInHistoricalChartayi]
create   VIEW [dbo].[P_CalcDayCountInHaghighiHoghughiHistoricalChartayi]
AS
with cte as (
select distinct cast(createDate as date) CreateDate
from HistoricalChartayi
where [خرید حقوقی] is not null),
cte365 as(
