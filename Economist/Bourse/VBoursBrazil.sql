...
from BoursBrazil b
--join BoursBrazilHistorical bh on b.Name = bh.Name
where not exists(select 1 from [dbo].BoursBrazil b2 where b.Name = b2.Name and b2.createdate > b.createdate) )
...
when [فعالیت شرکت ] not in (select [فعالیت شرکت ] from cte where [درصد تغییر نسبت به سال 2005] > 2.47 ) then N'Grade D-, فاقد مزیت نسبی و رقابتی, فوق العاده ضعیف, با سیاست انقباظی امریکا در معرض ریزش سهم, پیشنهاد فروش'
 ...
 [فعالیت شرکت ], [معرفی شرکت], [سال ورود ] from cte