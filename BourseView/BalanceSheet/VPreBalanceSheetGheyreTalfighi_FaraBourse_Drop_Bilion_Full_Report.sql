...
CREATE view [dbo].[VPreBalanceSheetGheyreTalfighi_FaraBourse_Drop_Bilion_Full_Report]
as
select distinct hc.InstrumentId,hc.symbolFA,
	...
	--,Cast(cf.['سود (زیان) عملیات در حال تداوم قبل از مالیات']/10000000000 as decimal(20,0))['سود (زیان) خالص عملیات در حال تداوم قبل از مالیات']
...
,cast(hc.['ذخایر و سایر بدهی ها']/10000000000 as decimal(20,0)) ['ذخایر و سایر بدهی ها']
	...
	--where 
	--not exists(select 1 from BalanceSheetGheyreTalfighiCategurbalancesheet_bourse h where hc.instrumentId = h.instrumentId and h.day > hc.day) and
	 --not exists(select 1 from BalanceSheetGheyreTalfighiCategurbalancesheet_bourse h where hc.instrumentId = h.instrumentId and h.createdate > hc.createdate) and
	 --not exists(select 1 from BalanceSheetGheyreTalfighiCategurbalancesheet_bourse h where hc.instrumentId = h.instrumentId and h.[تاریخ انتشار] > hc.[تاریخ انتشار]) and left(cast(hc.[تاریخ انتشار] as date) ,4) >= 2022 and
	--not exists(select 1 from BalanceSheetGheyreTalfighiCategurIncomeStatement_bourse h where cf.instrumentId = h.instrumentId and h.day > cf.day) and
	 --not exists(select 1 from BalanceSheetGheyreTalfighiCategurIncomeStatement_bourse h where cf.instrumentId = h.instrumentId and h.createdate > cf.createdate) and
	 --not exists(select 1 from BalanceSheetGheyreTalfighiCategurIncomeStatement_bourse h where cf.instrumentId = h.instrumentId and h.[تاریخ انتشار] > cf.[تاریخ انتشار])and left(cast(cf.[تاریخ انتشار] as date) ,4) >= 2022 
