...
--select * from VBoursAustralia_Denormal
 CREATE   view  [dbo].[VBoursAustralia_Denormal]    as with cte as (  select b.Name, b.FullName , Gheymat [شاخص], YearDarsad [درصد تغییر نسبت با سال گذشته],bh.[شاخص  بورس استرالیا   ۲۰۰۵ ] [شاخص بورس ۲۰۰۵ ],(b.Gheymat - bh.[شاخص  بورس استرالیا   ۲۰۰۵ ]) /bh.[شاخص  بورس استرالیا   ۲۰۰۵ ]*100 [درصد تغییر نسبت به سال 2005],null as [رتبه بندی شرکت],bh.[فعالیت شرکت ] [فعالیت شرکت ], bh.[معرفی شرکت] [معرفی شرکت], bh.[سال ورود ] [سال ورود ], nba.[رشد نسبت به سال 2005] [رشد نرخ برابری ارز نسبت به سال 2005], v.[Net income(FY)] [سود خالص], v.[Market cap] [حجم بازار],  v.InvestmentReturnPeriod [دوران بازگشت سرمایه],b.CreateDate
 from BoursAustralia b  
 join BoursAustraliaHistorical bh on b.Name = bh.Name  
 left join NerkheBarabariHistory na on na.Name like N'%استرالیا%'
 left join vNerkheBarabari nba on nba.PName like N'%'+dbo.RetreiveVahed(substring(na.pname,1,3))+'%'
 left join VBoursViewAustralia v  on b.Name = v.symbol
 where not exists(select 1 from [dbo].BoursAustralia b2 where b.Name = b2.Name and b2.createdate > b.createdate) )
...
[رتبه بندی شرکت],     [فعالیت شرکت ], [معرفی شرکت], N'('+cast([سال ورود ] as nvarchar(200))+N')' [سال ورود ], CreateDate
	  from cte  
